...
bool MidiFile::read(std::istream& input) {
    ...
    if (shortdata >= 0x8000) {
        ...
        case 29:  framespersecond = 29; break; // really 29.97 for color
        // television
        ...
        // << std::endl;
        // std::cerr << "SMPTE frames per second: " << framespersecond <<
        // std::endl;
    ...
    for (int i = 0; i < tracks; i++) {
        ...
        while (!input.eof()) {
            ...
            // std::cout << "command = " << std::hex << (int)event.data[0] <<
            // std::dec << std::endl;
...
void MidiFile::writeVLValue(long aValue, std::vector<uchar>& outdata) {
    ...
    bytes[3] = (uchar)(((ulong)aValue)       & 0x7f); // least significant 7
                                                      // bits
